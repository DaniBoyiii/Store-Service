...
def openaccount(admn): # opens requirement page with respect to admission no provided
    members.seek(0)
    stu = list(read_mem)[1:]
    members.seek(0)
    requirement.seek(0)
...
def order(info, dic, grade, maxm): # adds requirements into the file per class
    ...
    f[0].close()
    messagebox.showinfo('YASHWARDHAN DANI', 'Order Placed and logged out Successfully')
    step1()
...
def checkmember(dob_l, dob_d, dob_m, dob_y, btncontinue, ladmn, lclass, lnamef, lnames, admnentry, classbox, namefentry,
                namesentry, btnback): # checks valid entries for signup page and signs up
    if classbox.get() not in grades:
        messagebox.showerror('YASHWARDHAN DANI', 'Enter valid value for Class')
    elif int(dob_d.get()) not in dates:
        messagebox.showerror('YASHWARDHAN DANI', 'Enter valid value for Date')
    elif dob_m.get() not in months:
        messagebox.showerror('YASHWARDHAN DANI', 'Enter valid value for Month')
    elif int(dob_y.get()) not in years:
        messagebox.showerror('YASHWARDHAN DANI', 'Enter valid value for Year')
    else:
        try:
        ...
        try:
            if admn < 1 or admn > 1500:
                messagebox.showinfo('YASHWARDHAN DANI', 'Admission Number is not Valid')
                ...
                if not flag:
                    if len(namef) == 0:
                        messagebox.showinfo('YASHWARDHAN DANI', 'You can\'t Leave \'Name\' field empty !')
                        ...
                        members.flush()
                        messagebox.showinfo('YASHWARDHAN DANI', 'Registered Succesfully !')
                        step1()
                else:
                    messagebox.showinfo('YASHWARDHAN DANI', 'This Admission Number is Already Registered !')
                    admnentry.configure(bg='red')
        except ValueError:
            messagebox.showinfo('YASHWARDHAN DANI', 'Please Enter A Valid Number for Admission number!')
            admnentry.configure(bg='red')


def step22(): #login page
    ...
    psd = Label(w, border=5, text='Password (DDMMYYYY):', font=('Agency FB', 22, ' bold'), fg='black')
...
def logincheck(btnback, usr, usrentry, btnenter, psd, psdentry): #checking for valid entries after login page
    members.seek(0)
    adm = 0
    pswd = 0
    stu = list(read_mem)[1:]
    ...
    for i in stu:
        try:
            ...
            messagebox.showinfo('YASHWARDHAN DANI', 'Username and password shoud be in numeric form!')
        except IndexError:
            continue
    if adm == 1:
        if pswd == 1:
            ...
            openaccount(usrentry.get()) # goes to requirement page i.e logged in successfullyy
            psd.place_forget()
            psdentry.place_forget()
        else:
            messagebox.showinfo('YASHWARDHAN DANI', 'Incorrect Password!')
            psdentry.configure(bg='red')
    else:
        messagebox.showinfo('YASHWARDHAN DANI', 'You are not Registerd yet !')
        usrentry.configure(bg='red')

def step21():#signup page
    ...
    btncontinue = Button(w, text='Continue', font='arial 18', bg='blue', fg='white', width=10, border=5,
                         command=lambda: [checkmember(ldob, dob_d, dob_m, dob_y, btncontinue, ladmn, lclass, lnamef,lnames, admnentry, classbox, namefentry, namesentry, btnback)])
    btncontinue.place(x=980, y=600)


def step1(): #first_page (with singup and login options)
...
l3 = Label(w, text='SOFTWARE', font=('Monotype Corsiva', 18, 'bold'),
